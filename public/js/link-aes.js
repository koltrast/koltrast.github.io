...
document.addEventListener("DOMContentLoaded", function () {
  const keyText = "_-754m6yhOTJC2_X020FUih4it36nW24_i75ybFZFZA";
...
Aes.getKey = async function (keyText) {
  ...
  const key = window.crypto.subtle.importKey("raw", keyBytes, options, false, [
    "encrypt",
    "decrypt",
  ]);
  ...
  for (let i = 0; i < bytes.length; ++i) {
    ...
    if (r === 0) {
      text +=
        ...
        Base64.__getSymbol(((bytes[i] << 4) & 63) | (bytes[i + 1] >> 4));
    } else if (r === 1) {
...
AesLinkCoder.prototype.decode = function (node) {
  // Check if the node has the 'pgp' class
  if (node.classList.contains("pgp")) {
    // For '.pgp' class elements, handle as PGP key search
    Aes.decrypt(node.getAttribute("href"), this.key).then((decryptedText) => {
      const pgpSearchUrl =
        "https://keys.openpgp.org/search?q=" +
        encodeURIComponent(decryptedText);
      node.setAttribute("href", pgpSearchUrl);
      node.innerText = decryptedText;
    });
  } else {
    // For other elements, handle as a mailto link
    Aes.decrypt(node.getAttribute("href"), this.key).then((decryptedText) => {
      node.setAttribute("href", "mailto:" + decryptedText);
      node.innerText = decryptedText;
    });
  }
};
...
AesLinkCoder.prototype.encode = function (node) {
  Aes.encrypt(node.getAttribute("href"), this.key).then((encryptedText) => {
    node.setAttribute("href", encryptedText);
    node.innerText = encryptedText;
  });
};
